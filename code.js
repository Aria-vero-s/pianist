...
document.addEventListener('DOMContentLoaded', function() {
  const translations = {
    english: {
      about: {
        h1: "Hello, I'm Paolo",
        p: "I started playing the piano in Naples when I was 9, and I perfected my technique through classical music studies in Italy, before moving to France to study modern harmony and jazz. In Paris I completed a master’s degree in music and philosophy at the École Normale Supérieure de Paris, with a dissertation on the aesthetics of music and French structuralism. In the meantime, I focused my playing on jazz and started my career as a professional pianist in Parisian venues. Embracing a harmonious blend of musical traditions, my style evolved to be a unique fusion of Southern Italian and French music legacies, interwoven with a deep appreciation for the rhythmic richness of Brazilian music. This international approach to music eventually led me to direct the first World Music workshop at the renowned Cité Internationale Universitaire de Paris, Europe’s largest international student campus, where my diverse influences culminated into a celebration of global musical diversity."
      },
      lessons: {
        h1: "Lessons",
  ...
  function updateNavbarContent(navbarData) {
    const navbar = document.getElementById('navbar');
    const navbarLinks = navbar.querySelectorAll('ul.navbar-links li a');

    navbarLinks[0].textContent = navbarData.home;
    navbarLinks[1].textContent = navbarData.about;
    navbarLinks[2].textContent = navbarData.lessons;
    navbarLinks[3].textContent = navbarData.contact;
  }
